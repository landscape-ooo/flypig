...
switch($ts){
		...
		if($_FILES['picfile']){
			...
			if($arrUpload){
				...
				if($filesize<=0){
					...
					tsDimg($arrUpload['url'],'user','75','75',$arrUpload['path']);
					tsDimg($arrUpload['url'],'user','48','48',$arrUpload['path']);
					tsDimg($arrUpload['url'],'user','18','18',$arrUpload['path']);
					
					//header('Location: '.tsUrl('user','set',array('ts'=>'face')));
					tsNotice("头像修改成功！", null, tsUrl('user','set',array('ts'=>'face')), true);
				}
	...
	case "setpwd":
		...
		//更新密码
		if(md5($theUser['salt'].$oldpwd) != $theUser['pwd']){
			tsNotice("旧密码输入有误！");
		}
	...
	case "setemail":
		...
		if($email != $strUser['email']){
			...
			if($emailNum > 0){
				tsNotice("Email帐号已经存在，请换个其他Email帐号！");
			}
			//更新Email
			$new['user']->update('user',array(
				'userid'=>$userid,
