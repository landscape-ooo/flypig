...
{
	//adan
	private $saveRoot = 'D:/wwwroot/dev/attached/'; //根目录（以 '/' 结尾）
	//private $saveDomain = 'http://att.flypig.com.cn/'; //URL（以 '/' 结尾）
	private $saveDomain = '/attached/'; //URL（以 '/' 结尾）
	///
...
//        "POST" => "文件大小超出 post_max_size 限制" ,
    ...
    public function getFileInfo()
    {
        return array(
            ...
            "name" => $this->fileName ,
            "url" => $this->saveDomain . $this->getSavePath() . '/' . $this->fileName ,//adan
            "size" => $this->fileSize ,
            "type" => $this->fileType ,
    ...
    {
        $folderStr = $this->saveRoot.$this->getSavePath();
        if ( !file_exists( $folderStr ) ) {
            if ( !mkdir( $folderStr , 0777 , true ) ) {
                return false;
            }
        }
        return $folderStr;
