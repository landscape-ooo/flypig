...
function pagination($count, $perlogs, $page, $url, $suffix = '') {
	...
	if ($page > 6)
		...
		$re = '<a href="' . $url . '1' . $suffix . '" title="首页">1</a> ...' . $re;
		///
	if ($page + 5 < $pnums)
		//adan分页样式
		//$re .= '... <a href="' . $url . $pnums . $suffix . '" title="尾页">&raquo;</a>';
		$re .= '... <a href="' . $url . $pnums . $suffix . '" title="尾页">'.$pnums.'</a>';
		///
	if ($pnums <= 1)
		$re = '';
	return $re;
 ...
 * @return string
 */
function sysTemplate($file) {
	$tplfile = 'app/system/html/' . $file . '.html';
	$objfile = 'cache/template/system/' . $file . '.tpl.php';
 ...
 */
function tsUpload($files, $projectid, $dir, $uptypes, $width='', $height='') {
	if ($files ['size'] > 0) {
