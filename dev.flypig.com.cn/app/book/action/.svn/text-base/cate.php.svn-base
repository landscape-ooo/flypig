...
$bookcates .= "      字母分类：&nbsp;\r\n";
$bookcates .= "      <a".(($az == 0) ? " class=\"on\"" : "")." href=\"".tsUrl('book','cate',array('cidset'=>'0_'.$age.'_'.$intel.'_'.$theme.'_'.$spec))."\">不限</a>\r\n";
$bookcates .= "    </span>\r\n";
$bookcates .= "    <span class=\"c\">\r\n";
for($i=ord("a");$i<=ord("z");$i++){
	$bookcates .= "      <a".(($az == $i) ? " class=\"on\"" : "")." href=\"".tsUrl('book','cate',array('cidset'=>$i.'_'.$age.'_'.$intel.'_'.$theme.'_'.$spec))."\">".strtoupper(chr($i))."</a>\r\n";
}
$bookcates .= "    </span>\r\n";
$bookcates .= "  </li>\r\n";
foreach($APP_book['categroup_guide'] as $ckey=>$citem){
	if($arrCate[$citem]){
		...
		$bookcates .= "      <a".((${$citem.'_tmp'} == 0) ? " class=\"on\"" : "")." href=\"".tsUrl('book','cate',array('cidset'=>$az.'_'.$age.'_'.$intel.'_'.$theme.'_'.$spec))."\">不限</a>\r\n";
		$bookcates .= "    </span>\r\n";
		$bookcates .= "    <span class=\"c\">\r\n";
		foreach($arrCate[$citem]['catesub'] as $key=>$item){
			${$citem} = $key;
			$bookcates .= "      <a".((${$citem.'_tmp'} == $item['cateid']) ? " class=\"on\"" : "")." href=\"".tsUrl('book','cate',array('cidset'=>$az.'_'.$age.'_'.$intel.'_'.$theme.'_'.$spec))."\">".$item['catename']."</a>\r\n";
		}
		$bookcates .= "    </span>\r\n";
		$bookcates .= "  </li>\r\n";
		${$citem} = ${$citem.'_tmp'};
...
$url = tsUrl('book','cate',array('cidset'=>$cidset,'page'=>''));
...
//查询结果
$arrFilterBook = array();
$arrBooks = $new['book']->findAll('book',$cate_sql,'isrecommend desc,addtime asc','bookid',$lstart.','.$pagesize);
