...
function collectBook(bookid){
	var _url = siteUrl+'index.php?app=book&ac=ajax&ts=collect';
	$.ajax({
		url: _url,
		type: 'POST',
		data: {'bookid':bookid},
...
//想读正读已读操作
function doBook(bookid,type){
	var _url = siteUrl+'index.php?app=book&ac=ajax&ts=do';
	$.ajax({
		url: _url,
		type: 'POST',
...
function loveTopic(tid){
	...
	var url = siteUrl+'index.php?app=book&ac=do&ts=topic_collect';
...
function taoalbum(topicid){
	$.post(siteUrl+'index.php?app=book&ac=album&ts=topic',{'topicid':topicid},function(rs){
		if(rs==0){
			art.dialog({
				content : '请登陆后再进行淘帖',
				time : 1000
...
function recomment(rid,tid,token){
	...
	var url = siteUrl+'index.php?app=book&ac=do&ts=recomment';
...
function loadTopic(userid,page){
	...
	$.get(siteUrl+'index.php?app=book&ac=ajax&ts=topic',{'userid':userid,'page':page},function(rs){
		if(rs==''){
...
function topicAudit(topicid){
	$.post(siteUrl+'index.php?app=book&ac=ajax&ts=topicaudit',{'topicid':topicid},function(rs){
		if(rs==0){
			art.dialog({
				content : '书评取消审核成功！',
				time : 1000
...
//设为管理员，取消管理员
function setAdmin(bookid,userid,token){

	$.post(siteUrl+'index.php?app=book&ac=user&ts=manager',{'bookid':bookid,'userid':userid,'token':token},function(rs){
...
//踢出图书
function kickedBook(bookid,userid){
	$.post(siteUrl+'index.php?app=book&ac=kicked',{'bookid':bookid,'userid':userid},function(rs){
		if(rs=='0'){
			art.dialog({
				content : '非法操作',
				time : 1000
